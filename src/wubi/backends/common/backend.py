...
#
# Change Logs:
# 2011-05-13 Changed for Archlive     Carbon Jiao
...
class Backend(object):
    ...
    def modify_grub_configuration(self):
        ...
        dic = dict(
            ...
            rootflags = rootflags,
            title1 = "Completing the Archlive installation.",
            ...
            timezone = self.info.timezone,
            root = unix_path(self.info.disks_dir) + '/' + 'root.disk' ,
            rootsize = self.info.root_size_mb + 'MB' ,
            swap = unix_path(self.info.disks_dir) + '/' + 'swap.disk' ,
            swapsize = self.info.swap_size_mb + 'MB' ,
            #home = unix_path(self.info.disks_dir) + '/' + 'home.disk' ,
            #homesize = self.info.home_size_mb + 'MB' ,
            #usr = unix_path(self.info.disks_dir) + '/' + 'usr.disk' ,
            #usrsize = self.info.usr_size_mb + 'MB' ,
            )
        content = template
        for k,v in dic.items():
            k = "$(%s)" % k
        ...
        # Special for Archlive
        source = join_path(self.info.data_dir, 'grub.final.cfg')
        target = join_path(self.info.install_boot_dir, "grub", "grub.final.cfg")
        copy_file(source, target)
