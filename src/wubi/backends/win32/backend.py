...
# 2011-05-13 Changed by Carbon Jiao: Special for Archlive
...
class WindowsBackend(Backend):
    ...
    def undo_bootloader(self, associated_task):
        winboot_files = ['archldr', 'archldr.mbr', 'archldr.exe']
    ...
    def modify_bootini(self, drive, associated_task):
        ...
        if not os.path.isfile(bootini):
            ...
            return
        src = join_path(self.info.root_dir, 'winboot', 'archldr')
        dest = join_path(drive.path, 'archldr')
        shutil.copyfile(src,  dest)
        src = join_path(self.info.root_dir, 'winboot', 'archldr.mbr')
        dest = join_path(drive.path, 'archldr.mbr')
        shutil.copyfile(src,  dest)
        run_command(['attrib', '-R', '-S', '-H', bootini])
        boot_line = 'C:\archldr.mbr = "%s"' % self.info.distro.name
    ...
    def undo_bootini(self, drive, associated_task):
        ...
        if not os.path.isfile(bootini):
            return
        run_command(['attrib', '-R', '-S', '-H', bootini])
        remove_line_in_file(bootini, 'c:\archldr.mbr', ignore_case=True)
        run_command(['attrib', '+R', '+S', '+H', bootini])
    ...
    def modify_configsys(self, drive, associated_task):
        ...
        if not os.path.isfile(configsys):
            return
        src = join_path(self.info.root_dir, 'winboot', 'archldr.exe')
        dest = join_path(drive.path, 'archldr.exe')
        shutil.copyfile(src,  dest)
    ...
    def modify_bcd(self, drive, associated_task):
        ...
        mbr_path = join_path(self.info.target_dir, 'winboot', 'archldr.mbr')[2:]
